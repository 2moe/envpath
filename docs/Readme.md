...
The original design intent of **envpath** was to solve cross-platform directory configuration issues.
...
The Raw-format EnvPath is essentially an array structure that employs special parsing rules.

Its drawbacks are particularly evident: for average users, this format may appear less readable than string-based representations.

Note that user configuration files are meant to be human-readable, not just for deserialization purposes, so readability is crucial.

When not using special rules, it remains compatible with ordinary path arrays.

For example:
`["C:", "\\", "Users", "Public"]` is equivalent to `C:\Users\Public`
> There's an easily overlooked detail here: the second element is "\\".

Q: Special rules?
A: When an array element starts with `$[keyword]` and the entire expression can be properly parsed, it becomes a special rule. A single array may apply multiple special rules.

Examples:

- **Environment variables** (keyword `env`):
  `["$env:XDG_CONFIG_HOME", "xx"]` would resolve to `/home/[user]/.config/xx` on certain systems
- **`?` represents fallback**:
  - `["$dir:dl ? doc"]` specifies the Downloads directory (path varies across platforms)
  - If the Downloads directory value doesn't exist, use the Documents directory

Note: There's a distinction between **one `?`** and **two `?`**, which we'll address later.
