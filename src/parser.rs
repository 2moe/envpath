  ...
  iter
    ...
    .fold(Some(PathBuf::with_capacity(16)), |acc, s| {
      acc.and_then(|acc_p| {
        ...
        // returned. Note: crate::os_cow::from_str(s) is the default value.
        // `s` is the raw str.
        ...
        match len {
          ...
          // If the first element is $env, get the value of the environment
          // variable with the second element as the key
          _ => match chunks[0] {
            "$env" => {
              match get_2nd_chunk() {
                x if x.contains('*') => {
              ...
              // handle_env: Parsing environment variables (e.g.: `$env: home` or
              // `$env: userprofile ?? home`)
              or_default(EnvPath::handle_envs(&casing), s)
            }

            // If the first element is $const and the consts feature is enabled,
            // get the value of the directory with the second element
            // as the key
            #[cfg(feature = "consts")]
            "$const" => or_default(EnvPath::handle_consts(get_2nd_chunk()), s),
            #[cfg(feature = "value")]
            "$val" => or_default(EnvPath::handle_values(get_2nd_chunk()), s),
            // If the first element is $dir and the base-dirs feature is enabled,
            // get the value of the base directory with the second
            // element as the key
            #[cfg(feature = "dirs")]
            "$dir" => or_default(EnvPath::handle_dirs(get_2nd_chunk()), s),
            // If the first element starts with `$proj` and the `project` feature
            // is enabled, get the value of the project directory
            // with the second element as the key
            #[cfg(feature = "project")]
...
impl EnvPath<'_> {
  ...
  ///
  /// In addition to deserializing the configuration file, you can parse the
  /// vector or array.
  ///
  /// # Examples
  ///
...
/// If someone forgets to switch the Chinese input method to English, it is easy
/// to type ':' as '：', the two characters are particularly similar. To solve
/// the confusion problem, it supports both.
