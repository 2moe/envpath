...
use crate::{EnvPath, OsCow, parser};
...
impl EnvPath<'_> {
  pub(crate) const START_ARR: [&'static str; 5] =
    ["env", "dir", "const", "proj", "val"];
  /// It's a function for parsing rules(e.g. `$env: user ? userprofile ??
  /// home`). The `s` parameter in this function refers to all strings in the
  /// closed interval from **user** to **home**. Does not contain the `$env:`.
  ///
  /// Design ideas explained:
  ///
  /// Split str first, terminator is `?` or `？`.(It depends on `separator`)
  /// Assume that the element of each iteration is x, and that the element of
  /// the previous iteration, after processing (e.g., passing in the value
  /// generated by env::var_os()) is acc.
  //// If acc is None, and x is empty. This means that the value of the previous
  //// environment variable does not exist, and this time a double check is required
  //// (not only must the previous env exist, but the path to it must also exist).
  //// Since the condition is not met, continue to the next iteration.
  ///
  /// If acc is None and x is not empty, then check the value of x's environment
  /// variable.
  ///
  /// If acc is Some, and x is not empty, exit the iterator. (If the value of
  /// the previous environment variable exists, we do not check the value of x
  /// this time, but return the value of the previous one)
  ///
  /// If acc is Some and x is empty, then determine if the file exists. If it
  /// does, we exit the iterator. If not, then acc is None.
  ...
  ///
  /// If someone forgets to switch the Chinese input method to English, it is
  /// easy to type '?' as '？'.
  ...
  /// This function is used to handle ident starting with `env *` or `env*`, and
  /// then resolve the environment variable to the right of `*`
  ///
  /// Assuming that the ident is `env * home`, it does not automatically convert
  /// `home` to `HOME`, but gets `$home` directly.
  ...
  pub(crate) fn handle_envs(ident: &str) -> OsCow {
    ...
    match Self::get_question_mark_separator(ident) {
      ' ' => var_os(ident).and_then(crate::os_cow::into_os_cow),
